-- MsDevTools Spoon — logging, dev panels, and branch tracing.
-- Extracted from ms_core.lua §1 (dev log infra), §3 (watcher/tracing),
-- and §12 (developer panels).
...
MsDevTools.branchTrace  = true  -- enable/disable branch tracing

-- ═══════════════════════════════════════════════════════════════════
-- Internal state (populated in init/start)
-- ═══════════════════════════════════════════════════════════════════
...
-- Panel references (set when panels are built)
...
local _mousePos, _mousePoller, _windowPoller, _windowHistory, _windowLast, _windowMaxHistory
local _pushMouseState

-- Watcher/tracing state
...
function MsDevTools:init()
    ...
    -- Archive old logs on every reload
...
function MsDevTools:start()
    -- Populate ms.dev with the public API surface.
    -- Called after ms.* globals are ready.
    if not ms then return end

    ms.dev = {
        _consolePanel    = nil,  -- populated by _buildConsolePanel()
        _watcherPanel    = nil,  -- populated by _buildWatcherPanel()
        _keysPanel       = nil,  -- populated by _buildKeysPanel()
        ...
        _keysReady       = false,  -- set true by Keys panel nav callback
    }

    -- Use a metatable so ms.dev._watcherPanel etc. always reflect
    -- the Spoon's internal panel references (which change when panels
    -- are built/destroyed).
    setmetatable(ms.dev, {
    ...
    -- Public logging API
    ms.dev.log = function(entry) self:log(entry) end

    -- Event hooks (called from ms_core.lua eventtap callbacks)
    ms.dev._onMacroFire  = function(...) self:onMacroFire(...) end
    ms.dev._onKeyEvent   = function(...) self:onKeyEvent(...) end
    ms.dev._onMouseEvent = function(...) self:onMouseEvent(...) end
    ...
    ms.dev._pushMouseState = function(x, y) self:pushMouseState(x, y) end
    ...
    _G.print = function(...)
        ...
        self:log({ type = "print", msg = table.concat(parts, "\t") })
    end
end
...
function MsDevTools:_archiveOnReload()
    ...
    for _, p in pairs(_catPaths) do self:_archiveLog(p, stamp, "json") end
    for _, p in pairs(_readablePaths) do self:_archiveLog(p, stamp, "readable") end
...
function MsDevTools:_devWrite(entry)
    ...
    -- Panel routing with consecutive-repeat suppression.
    local t = entry.type
...
-- Public API: any code can call MsDevTools:log({ type = "system", event = "...", ... })
...
function MsDevTools:onKeyEvent(keyCode, keyName, isDown)
    self:_devWrite({ type = "key", key = keyName or ("code:" .. tostring(keyCode)), keyCode = keyCode, down = isDown })
    if isDown then _activeKeys[keyCode] = keyName or tostring(keyCode)
    else            _activeKeys[keyCode] = nil end
    if _keysPanel then
        local active = {}
...
function MsDevTools:onMouseEvent(button, isDown, x, y)
    self:_devWrite({ type = "mouse", button = button, down = isDown, x = x, y = y })
    if isDown then _activeButtons[button] = true
    else            _activeButtons[button] = nil end
    if _keysPanel and _keysReady then
        local active = {}
        for btn in pairs(_activeButtons) do table.insert(active, btn) end
...
function MsDevTools:flushCam()
    if _camMoveAccum > 0 then
        if _watcherPanel then
            self:watcherStep("cam.move ×" .. _camMoveAccum)
        end
        self:macroLog("cam.move ×" .. _camMoveAccum)
        _camMoveAccum = 0
    end
end
...
-- Accumulator accessors (called from ms.press/release/wait/cam.move)
...
-- Trace suppress (used by ms.type to suppress watcher logging during internal dispatch)
...
-- Mouse state pusher (used by Keys panel)
...
function MsDevTools:_buildConsolePanel()
    ...
    local panel = hs.webview.new({ x=x, y=y, w=w, h=h },
        { developerExtrasEnabled = true }, ucCon)
...
function MsDevTools:showConsole()
    ...
    hs.timer.doAfter(0.1, function()
        ...
        local tj = _devThemeJS()
        if tj ~= "" then pcall(function() _consolePanel:evaluateJavaScript(tj) end) end
    end)
end
...
function MsDevTools:toggleConsole()
    if _consoleOpen then self:hideConsole()
    else self:showConsole() end
end
...
function MsDevTools:_buildWatcherPanel()
    ...
    local y = screen.y + 44
    local panel = hs.webview.new({ x=x, y=y, w=w, h=h },
        { developerExtrasEnabled = true }, ucWatcher)
    if not panel then return nil end
    pcall(function() panel:windowStyle(0) end)
    pcall(function() panel:level(hs.canvas.windowLevels.floating) end)
    pcall(function() panel:behavior(hs.canvas.windowBehaviors.canJoinAllSpaces) end)
...
function MsDevTools:showWatcher()
    ...
    hs.timer.doAfter(0.1, function()
        ...
        local tj = _devThemeJS()
        if tj ~= "" then pcall(function() _watcherPanel:evaluateJavaScript(tj) end) end
    end)
end
...
function MsDevTools:toggleWatcher()
    if _watcherOpen then self:hideWatcher()
    else self:showWatcher() end
end
...
function MsDevTools:_buildKeysPanel()
    ...
    local y = screen.y + 68
    local panel = hs.webview.new({ x=x, y=y, w=w, h=h },
        { developerExtrasEnabled = true }, ucKeys)
    if not panel then return nil end
    pcall(function() panel:windowStyle(0) end)
    pcall(function() panel:level(hs.canvas.windowLevels.floating) end)
    pcall(function() panel:behavior(hs.canvas.windowBehaviors.canJoinAllSpaces) end)
...
function MsDevTools:showKeys()
    ...
    hs.timer.doAfter(0.1, function()
        ...
        pcall(function() _pushMouseState() end)
        local tj = _devThemeJS()
        if tj ~= "" then pcall(function() _keysPanel:evaluateJavaScript(tj) end) end
    end)
...
function MsDevTools:toggleKeys()
    if _keysOpen then self:hideKeys()
    else self:showKeys() end
end
...
function MsDevTools:_buildWindowPanel()
    ...
    local y = screen.y + 68
    local panel = hs.webview.new({ x=x, y=y, w=w, h=h },
        { developerExtrasEnabled = true }, ucWindow)
    if not panel then return nil end
    pcall(function() panel:windowStyle(0) end)
    pcall(function() panel:level(hs.canvas.windowLevels.floating) end)
    pcall(function() panel:behavior(hs.canvas.windowBehaviors.canJoinAllSpaces) end)
...
function MsDevTools:toggleWindow()
    if _windowOpen then self:hideWindow()
    else self:showWindow() end
end
...
-- ═══════════════════════════════════════════════════════════════════
-- Public accessors for panel references (used by frozenMs proxy etc.)
-- ═══════════════════════════════════════════════════════════════════
