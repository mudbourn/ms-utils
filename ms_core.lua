...
-----END PUBLIC KEY-----
            ...
            local function _devWrite(entry)
                ...
                if ok then
                    local t = entry.type
                    -- Console: all types except mousemove (too high-frequency).
                    -- key/mouse now appear with KEY/MOUSE badges instead of a burst notice.
                    if ms.dev._consolePanel and t ~= "mousemove" then
                        pcall(function()
                            ms.dev._consolePanel:evaluateJavaScript("appendEntry(" .. json .. ")")
                        end)
                    end
                    -- Watcher: macro, print, error (step entries are pushed directly)
