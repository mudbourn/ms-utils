...
;
; Loading chain:  init.ahk → _ms_main.ahk → (hash check) → ms_core.ahk
;
; Usage:
;   #Include ms_core.ahk  (done by _ms_main.ahk after hash verification)
...
; ── Reference defaults (mirror macOS REF_W / REF_H) ──────────────────────────
global REF_W     := 1680
global REF_H     := 1044
global REF_SENS  := 1.5
...
global _ms_running    := Map()    ; groupId → true (cooldown)
global _ms_binds      := Map()    ; id → enabled
global _ms_bindConfig := Map()    ; id → bind override
global _ms_subBinds   := Map()    ; sub-id → bind
global _ms_modConfig  := Map()    ; id → mod key
global _ms_cooldowns  := Map()    ; id → cooldown ms
...
global _ms_trackpad_bind_ovr  := Map()  ; id → bind override in trackpad mode
global _ms_independent_binds  := false

; ── Menu / features ──────────────────────────────────────────────────────────
global _ms_menu_defs    := []      ; ms.menu.define entries
...
; ═══════════════════════════════════════════════════════════════════════════════
;  Section 3 — Helpers (coordinate, keyboard, bind utils, settings, UI)
; ═══════════════════════════════════════════════════════════════════════════════
...
; ── Hotkey fire callbacks (used by ms.bind.rebind) ─────────────────────────
...
; ── Notify (state-change toast + sound) ─────────────────────────────────────
...
; ── Toast system (Gui-based, matches macOS canvas toasts) ──────────────────
...
; ── Loading screen (WebView2 — same HTML as macOS) ──────────────────────────
...
; ── App watcher (poll for Roblox window) ───────────────────────────────────
...
; ── System hotkey registration (Alt+[/]/P/F10, /, Enter) ───────────────────
...
; ── Integrity auto-seed at startup ──────────────────────────────────────────
...
; ── Dev log archive (prune old logs on startup) ─────────────────────────────
...
; ── UI state builder (for WebView2 settings panel) ────────────────────────
