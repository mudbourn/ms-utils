-- Core System ---- PLEASE EDIT CAREFULLY --
    -- Hammerspoon mudscript Utility Library --
        ...
        -- 2. Conditions, States, and UI Elements--
            ...
            -- Settings Menu --
                ...
                auditMacros = function(src)
                    -- Pass 1: strip block comments, preserving newlines so that
                    -- line numbers stay stable for the global-function check below.
                    local parts, i, n = {}, 1, #src
                    while i <= n do
                        local p = src:find("%-%-", i)
                        if not p then
                            parts[#parts + 1] = src:sub(i); break
                        end
                        parts[#parts + 1] = src:sub(i, p - 1)
                        local j = p + 2
                        local handled = false
                        if src:sub(j, j) == "[" then
                            local eq = 0
                            ...
                            if src:sub(j + 1 + eq, j + 1 + eq) == "[" then
                                ...
                                local cs, ce = src:find(closer, j + 2 + eq, true)
                                if cs then
                                    parts[#parts + 1] = src:sub(p, ce):gsub("[^\n]", " ")
                                    i = ce + 1
                                    handled = true
                                end
                            end
                        ...
                        if not handled then
                            -- Plain line comment: skip to newline, keep the newline itself.
                            local nl = src:find("\n", j)
                            i = nl or (n + 1)
                        end
                    end
                    -- Pass 2: strip any remaining line comments; prepend a space.
                    local clean = " " .. table.concat(parts):gsub("%-%-[^\n]*", "")
                    local errs  = {}
                ...
                -- Compare the current init.lua hash to the stored trusted baseline.
                -- Returns: status ("trusted"|"mismatch"|"uninitialized"), currentHash, trustedHash
                ms.integrity.check = function()
                    local cur     = ms.integrity.hashFile(initPath)
                ...
                -- Seal the current init.lua as the trusted baseline.
                ms.integrity.trustCurrent = function()
                    local hash = ms.integrity.hashFile(initPath)
                    if not hash then
                        ms.alert("Integrity: could not hash init.lua.", 4)
                        return false
                    end
                ...
                -- downloaded init.lua against it, back up the current file, write the new one,
                ...
                ms.integrity.update = function()
                    ...
                    if not manifestURL or manifestURL == "" then
                        ms.alert("Update URL not configured.\nSet ms._updateManifestURL in init.lua.", 6)
                        return
                    end
                    ...
                    hs.http.asyncGet(manifestURL, nil, function(mCode, mBody, _)
                        ...
                        hs.http.asyncGet(manifest.url, nil, function(fCode, fBody, _)
                            ...
                            -- Write to a temp file so we can hash it before touching init.lua.
                            os.execute("mkdir -p '" .. archivePath .. "'")
                            local tmpPath = archivePath .. "init_update_tmp.lua"
                            ...
                            -- Back up current init.lua.
                            local timestamp  = os.date("%Y-%m-%d_%H%M")
                            local backupFile = archivePath .. "init_" .. timestamp .. ".lua.bak"
                            local bOk = moveFile(initPath, backupFile)
                            if not bOk then
                                os.remove(tmpPath)
                                ms.alert("Update failed: could not back up init.lua.", 4)
                                return
                            end
                            -- Move verified temp into place.
                            local mOk = moveFile(tmpPath, initPath)
                            if not mOk then
                                moveFile(backupFile, initPath)  -- restore
                                ms.alert("Update failed: could not install new init.lua.\nBackup restored.", 5)
                                return
                            end
                ...
                local _legacyNativeMenuBuilder = function()
                                ...
                                if parsed then
                                    ...
                                    if conflictId then
                                        ...
                                        hs.dialog.blockAlert(
                                            "Bind Conflict",
                                            "\"" .. bindStr2 .. "\" is already used by \"" .. cLabel .. "\".\n\nChoose a different input to avoid the conflict.",
                                            "OK"
                                        )
                                        local roblox = hs.application.get("Roblox")
                                        if roblox then roblox:activate() end
                                        ms.alert("Rebind cancelled — conflict with \"" .. cLabel .. "\". Try a different input.", 4)
                                        return true
                                    end
                                    ms.playSlot("alert")
                                    hs.focus()
                    ...
                    local function makeModRebindFn(item, parentLabel)
                        return function()
                            ...
                            capture = hs.eventtap.new({ hs.eventtap.event.types.keyDown }, function(event)
                                ...
                                local keyCode = event:getKeyCode()
                                if keyCode == 53 then ms.alert("Rebind cancelled.", 2); return true end
                                if keyCode == 51 then
                                    ms.playSlot("alert")
                                    hs.focus()
                                    local result = hs.dialog.blockAlert(
                                        "Clear Modifier",
                                        "Clear modifier for \"" .. item.label .. "\"?",
                                        "Clear", "Cancel"
                                    )
                                ...
                                if keyStr then
                                    ...
                                    if conflictId then
                                        ...
                                        hs.dialog.blockAlert(
                                            "Modifier Conflict",
                                            "\"" .. keyStr .. "\" is already used by \"" .. cLabel .. "\" under the same parent.\n\nChoose a different modifier key.",
                                            "OK"
                                        )
                                        local roblox = hs.application.get("Roblox")
                                        if roblox then roblox:activate() end
                                        ms.alert("Modifier rebind cancelled — conflict with \"" .. cLabel .. "\". Try a different key.", 4)
                                        return true
                                    end
                                    ms.playSlot("alert")
                                    hs.focus()
                                    local result = hs.dialog.blockAlert(
                                        "Confirm Modifier",
                                        "Set modifier for \"" .. item.label .. "\" to:  " .. keyStr,
                                        "Confirm", "Cancel"
                                    )
                                    if result == "Confirm" then
                        ...
                        return function()
                            ...
                            capture = hs.eventtap.new({
                                ...
                                if t == hs.eventtap.event.types.keyDown and event:getKeyCode() == 51 then
                                    ms.playSlot("alert")
                                    hs.focus()
                                    local r = hs.dialog.blockAlert("Clear Bind",
                                        "Clear independent bind for \"" .. item.label .. "\"?", "Clear", "Cancel")
                                    if r == "Clear" then
                                ...
                                if parsed then
                                    ...
                                    hs.focus()
                                    local result = hs.dialog.blockAlert(
                                        "Confirm Sub-item Bind",
                                        "Set \"" .. parentLabel .. " › " .. item.label .. "\" to:  " .. displayStr,
                                        "Confirm", "Cancel"
                                    )
                                    if result == "Confirm" then
                    ...
                    local function buildBindSection(defs)
                        ...
                        for _, bind in ipairs(defs) do
                            ...
                            table.insert(rebindSub, {
                                ...
                                fn = function()
                                    ...
                                    local btn, value = hs.dialog.textPrompt(
                                        "Set Cooldown",
                                        "Cooldown for \"" .. bind.label .. "\" (ms).\nDefault: " .. tostring(codeDefault) .. "ms  |  0 = no cooldown:",
                                        tostring(current), "Set", "Cancel"
                                    )
                                    local roblox = hs.application.get("Roblox")
                                    if btn == "Set" then
                        ...
                        table.insert(section, { title = "Reset All to Default...", fn = function()
                            ...
                            local result = hs.dialog.blockAlert(
                                ...
                                "Reset all binds and cooldowns in this section to their default values?",
                                "Reset", "Cancel"
                            )
                            local roblox = hs.application.get("Roblox")
                    ...
                    local function buildModifiersSubmenu()
                        ...
                        table.insert(sub, { title = "Reset All Modifiers to Default", fn = function()
                            ms.playSlot("alert")
                            hs.focus()
                            local result = hs.dialog.blockAlert("Reset Modifiers", "Reset all modifiers to their default values?", "Reset", "Cancel")
                            if result == "Reset" then
                                ms.modConfig = {}
                                ms.saveSettings()
                                ms.playSlot("reset")
                                local roblox = hs.application.get("Roblox")
                                if roblox then roblox:activate() end
                                hs.timer.doAfter(0.2, function() ms.alert("All modifiers reset to default.", 3, true) end)
                            else
                                local roblox = hs.application.get("Roblox")
                                if roblox then roblox:activate() end
                            end
                    ...
                    -- Independent Binds submenu --
                        ...
                        table.insert(sub, { title = "Reset All Binds to Default", fn = function()
                            ...
                            local result = hs.dialog.blockAlert(
                                "Reset Binds",
                                "Reset all binds to their default values?",
                                "Reset", "Cancel"
                            )
                            if result == "Reset" then
                                ms.bindConfig = {}
                    ...
                    local function buildSoundSubmenu()
                        ...
                        table.insert(sub, {
                            ...
                            fn = function()
                                ms.playSlot("alert")
                                hs.focus()
                                local btn, value = hs.dialog.textPrompt("Sound Volume", "Enter volume (0-100):", tostring(ms.soundVolume or 100), "Set", "Cancel")
                                local roblox = hs.application.get("Roblox")
                                if btn == "Set" then
                                    local num = tonumber(value)
                        ...
                        fn = function()
                            ...
                            capture = hs.eventtap.new({ hs.eventtap.event.types.keyDown }, function(event)
                                ...
                                if newKey then
                                    ms.playSlot("alert")
                                    hs.focus()
                                    local result = hs.dialog.blockAlert("Confirm Rebind", "Set Left Click Hold to:  " .. newKey, "Confirm", "Cancel")
                                    if result == "Confirm" then
                                        ...
                                        local roblox = hs.application.get("Roblox")
                                        if roblox then roblox:activate() end
                                        hs.timer.doAfter(0.2, function() ms.alert("Left Click Hold set to: " .. newKey, 3, true) end)
                                    else
                                        ms.alert("Rebind cancelled.", 2)
                                        local roblox = hs.application.get("Roblox"); if roblox then roblox:activate() end
                                    end
                                else
                                    ms.alert("Could not read key. Try again.", 2)
                                end
                        ...
                        fn = function()
                            ...
                            capture = hs.eventtap.new({ hs.eventtap.event.types.keyDown }, function(event)
                                ...
                                if newKey then
                                    ms.playSlot("alert")
                                    hs.focus()
                                    local result = hs.dialog.blockAlert("Confirm Rebind", "Set Right Click Hold to:  " .. newKey, "Confirm", "Cancel")
                                    if result == "Confirm" then
                                        ...
                                        local roblox = hs.application.get("Roblox")
                                        if roblox then roblox:activate() end
                                        hs.timer.doAfter(0.2, function() ms.alert("Right Click Hold set to: " .. newKey, 3, true) end)
                                    else
                                        ms.alert("Rebind cancelled.", 2)
                                        local roblox = hs.application.get("Roblox"); if roblox then roblox:activate() end
                                    end
                    ...
                    table.insert(sub, {
                        ...
                        fn = function()
                            ms.playSlot("alert")
                            hs.focus()
                            local result = hs.dialog.blockAlert("Reset Hold Keys", "Reset both hold keys to defaults?  ( n / j )", "Reset", "Cancel")
                            if result == "Reset" then
                                ...
                                ms.playSlot("reset")
                                local roblox = hs.application.get("Roblox"); if roblox then roblox:activate() end
                                hs.timer.doAfter(0.2, function() ms.alert("Hold keys reset to default  ( n / j )", 3, true) end)
                            else
                                local roblox = hs.application.get("Roblox"); if roblox then roblox:activate() end
                            end
                        end
                    ...
                    local function buildSettingsSubmenu()
                        return {
                            ...
                            { title = "Set Sensitivity...", fn = function()
                                ms.playSlot("alert")
                                hs.focus()
                                local btn, value = hs.dialog.textPrompt("Camera Sensitivity", "Enter your Roblox camera sensitivity:", tostring(CUR_CAM_SENS), "Set", "Cancel")
                                if btn == "Set" then
                                    local num = tonumber(value)
                            ...
                            { title = "-" },
                            {
                                ...
                                fn       = not _trusted and function()
                                    ...
                                    if status == "uninitialized" then
                                        prompt = "Seal this init.lua as the trusted baseline?\nHash: " .. (cur and cur:sub(1, 16) or "?") .. "\xe2\x80\xa6"
                                    else
                                        prompt = "Hash mismatch detected. Trust the CURRENT (possibly modified) version?\nHash: " .. (cur and cur:sub(1, 16) or "?") .. "\xe2\x80\xa6"
                                    end
                                    local r = hs.dialog.blockAlert("Trust Current Version", prompt, "Trust", "Cancel")
                                    local roblox = hs.application.get("Roblox")
                                    if r == "Trust" then ms.integrity.trustCurrent() end
                                    if roblox then roblox:activate() end
                                end or nil,
                            },
                        }
                        ...
                        return {
                            ...
                            { title = "Check Integrity", fn = function()
                                ...
                                if status == "trusted" then
                                    ms.alert("\xe2\x9c\x93 init.lua matches trusted hash.\n" .. (cur and cur:sub(1, 16) or "?") .. "\xe2\x80\xa6", 5, true)
                            ...
                            { title = "Check for Update...", fn = function()
                                if not ms._updateManifestURL or ms._updateManifestURL == "" then
                                    ms.alert("No update URL configured.\nSet ms._updateManifestURL in init.lua.", 5)
                                    return
                                end
                                ms.playSlot("alert")
                                hs.focus()
                                local r = hs.dialog.blockAlert(
                                    "Check for Update",
                                    "Download and apply the latest init.lua from GitHub?\n\nThe current file will be backed up to backups/ and Hammerspoon will reload.",
                                    "Update", "Cancel"
                                )
                                local roblox = hs.application.get("Roblox")
                                if roblox then roblox:activate() end
                                if r == "Update" then ms.integrity.update() end
                            end },
        ...
        -- 3. Keyboard Actions --
            ...
            ms._keyListener = hs.eventtap.new({
                ...
                -- Merged here from the old separate toggleWatcher eventtap, which fired
                -- on every system keyDown and called frontmostApplication() each time.
                -- ms._robloxActive costs nothing to check — it's a plain boolean.
                if type == hs.eventtap.event.types.keyDown and ms._robloxActive then
                        ...
                        if binding and binding.keyCode == keyCode then
                            ...
                            if modsMatch then
                                if BindValidity == 1 then
                                    if type == hs.eventtap.event.types.keyDown and binding.pressFn then
                                        local co = coroutine.create(binding.pressFn)
                                        local ok, err = coroutine.resume(co)
                                        if not ok then print("ms.key error: " .. tostring(err)) end
                                    elseif type == hs.eventtap.event.types.keyUp and binding.releaseFn then
        ...
        -- 9. Misc --
            ...
            ms.playSlot = function(slotId)
                ...
                -- Prevents double-play when e.g. a Return keypress and the item's fn
                -- both call playSlot("interact") within the same action, and prevents
                -- the AX hover observer from double-firing on rapid repeats.
                -- hs.timer.absoluteTime() returns uptime in seconds.
                ...
                local path
                if assigned then
                    -- soundAssign stores a sound name; resolve it to the full path via
                    -- ms.sounds. Falls back to treating it as a literal path so any
                    -- legacy configs that stored paths directly still work.
                    path = (ms.sounds and ms.sounds[assigned]) or assigned
                else
                    -- Auto mode: look for a file named after this slot in SoundLib.
        ...
        -- 11. Webview Settings Panel --
            ...
            -- effect on the next panel open without needing a full hs.reload().
            ...
            ms.ui._actions = {
                ...
                checkIntegrity = function()
                    ...
                    if status == "trusted" then
                        ms.alert("\xe2\x9c\x93 init.lua matches trusted hash.\n" .. (cur and cur:sub(1, 16) or "?") .. "\xe2\x80\xa6", 5, true)
                    elseif status == "mismatch" then
                        ms.alert("\xe2\x9a\xa0 Hash mismatch!\nExpected: " .. (trusted and trusted:sub(1, 16) or "?")
                            .. "\xe2\x80\xa6\nCurrent:  " .. (cur and cur:sub(1, 16) or "?") .. "\xe2\x80\xa6", 9)
                    else
                        ms.alert("No trusted hash on record.\nUse \"Trust Current Version\" to seed trust.", 5)
                    end
                    ms.ui.refresh()
                    ...
                    capture = hs.eventtap.new({
                        ...
                        if parsed then
                            ...
                            if conflictId then
                                ...
                                ms.playSlot("alert")
                                hs.focus()
                                hs.dialog.blockAlert("Bind Conflict",
                                    "\"" .. bindStr2 .. "\" is already used by \"" .. cLabel .. "\".\n\nChoose a different input.",
                                    "OK")
                                ms.alert("Rebind cancelled — conflict with \"" .. cLabel .. "\".", 4)
                                ms._inputOpen = false
                                restorePanel()
                                return true
                            end
                            ms.playSlot("alert")
                            hs.focus()
                            local result = hs.dialog.blockAlert("Confirm Rebind",
                                "Set \"" .. label .. "\" to:  " .. bindStr2,
                                "Confirm", "Cancel")
                            ms._inputOpen = false
                            if result == "Confirm" then
            ...
            -- Anchors the panel under the menu-bar icon when its on-screen frame is
            -- available; falls back to the center of the main screen otherwise.
            ...
            ms.ui.show = function()
                ...
                pcall(function() ms.ui._panel:frame(_panelFrame()) end)
                ms.ui._panel:show()
    ...
    -- Startup Executions --
            ...
            hs.timer.doAfter(0.5, function() ms.playSlot("load") end)
